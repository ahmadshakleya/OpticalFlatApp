...
def create_cylinder(optical_flat):
    min_z = np.min(optical_flat.get_cylinder().get_surface()[2])
    cylinder = Cylinder(height=10, radius=2, x_center=0, y_center=0, z_base=min_z, resolution=100)
    cylinder.translate(dz=-30)
    cylinder.tilt(theta=90, phi=0)
    return cylinder

def compute_intersections(optical_flat, cylinder):
...
def plot_results(x_int, y_int, z_int, cylinder_center):
    ...
    plt.savefig("optical_flat_visualization2.png", dpi=300, bbox_inches='tight', pad_inches=0.1)
    plt.close()

def main():
    optical_flat = create_optical_flat()
    cylinder = create_cylinder(optical_flat)
    cylinder_center = optical_flat.get_cylinder().get_center()
    x_int, y_int, z_int = compute_intersections(optical_flat, cylinder)
    plot_results(x_int, y_int, z_int, cylinder_center)
