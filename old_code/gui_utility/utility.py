...
def refine_stl_mesh(stl_path, resolution):
    ...
    # Assuming we simply double the resolution by splitting each triangle into 4
    ...
    for i, f in enumerate(your_mesh.data):
        ...
        # Create four new triangles from the original and three new points
        idx = i * 4
...
def make_circle(points):
    # Converted from JavaScript into Python from the algorithm by Welzl (1991)
    # It's a randomized algorithm that has expected linear time complexity.

    # Clone list to preserve the original order
    shuffled = points[:]
...
def rotate_y(coordinates, phi):
    ...
    phi = np.radians(phi)  # Convert degrees to radians
    rotation_matrix = np.array([
